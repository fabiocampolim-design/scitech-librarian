...
The tool reads its keys from the process environment. Two routes get them
there, and you can mix them:

- **A `.env` file** — copy `.env.example` to `.env` and fill it in; the script
  loads it automatically, no shell variables to set. It is gitignored.
- **Environment variables** — export them from your shell, set them in your
  agent's or launcher's configuration, or supply them as CI secrets. These take
  precedence: `.env` only fills in what is not already set, so if you configure
  the keys this way you never need a `.env` at all.

Either way, `python librarian.py --list` shows which keys arrived and
`--selftest` proves they work.
...
on Linux, Windows and macOS under Python 3.9 and 3.13.

## Claude Code skill

`SKILL.md` at the repository root is a [Claude Code](https://claude.com/claude-code)
skill that teaches an agent to run scitech-librarian from your clone — which
script does what, the key and query workflow, and the traps each database
sets. Install it by copying the file to
`~/.claude/skills/literature-search/SKILL.md`; the agent then finds the
clone through `SCITECH_LIBRARIAN_HOME` (if set) or by searching for
`librarian.py`, and never copies the scripts into a project. The test suite
checks that an installed copy is byte-identical to the shipped file, so the
skill cannot drift from the release it describes.
...
Clarivate (Web of Science, JCR), Semantic Scholar, Crossref, CORE,
Unpaywall or SCImago; their names are used only to identify the services it queries.
