...
- **Offline-testable.** 192 checks run with no network and no keys (backends
...
192 checks, stdlib only, no network and no keys — backends run against
...
| **Software** | — | All of it |
| **Validation** | Live novelty scans on real research queries; caught the WoS grammar traps, the arXiv hang, the OpenAlex/Scopus count discrepancy | The 192-check offline suite; CI; live selftests |
