# Literature search report -- run 20260907T102939

Generated by scitech-librarian 3.6.0 (report level: simple). Every number below is reproducible from the archived run directory `20260907T102939`.

| Item | Value |
|---|---|
| Run started | 2026-09-07 10:29:39 |
| Duration | 52 s |
...
| (n = 5,722)                              |     | automation tools (venue filter) (n = 36) |
| openalex: 4,972                          | --> | duplicates removed (n = 59)              |
| arxiv: 733                               |     +------------------------------------------+
| inspire: 17                              |
+------------------------------------------+
                      |
...
SCREENING
+------------------------------------------+     +------------------------------------------+
| Records screened (n = 1,230)             | --> | Records excluded (n = --)                |
+------------------------------------------+     +------------------------------------------+
                      |
                      v
+------------------------------------------+     +------------------------------------------+
...
| Stage | n |
|---|---|
| Records identified from databases | 5,722 |
|   openalex | 4,972 |
|   arxiv | 733 |
|   inspire | 17 |
| Records retrieved (downloaded / ingested) | 1,325 |
| Removed before screening: automation (non-curated venues) | 36 |
| Removed before screening: duplicates | 59 |
| Records to screen (unique) | 1,230 |
| Records screened | 1,230 (assumed = unique) |
...
| 11 | Prior work | none |
| 12 | Updates | 1 earlier run(s) archived; counts tracked in counts_history.csv |
| 13 | Dates of searches | searched on 2026-09-07 10:29:39 |
| 14 | Peer review | none |
| 15 | Total records | 5,722 identified from databases; 1,325 retrieved; 1,230 unique |
| 16 | Deduplication | exact DOI match, else first 90 characters of the lower-cased title; 59 duplicates removed |

## Top 10 records per block

Deduplicated across sources, sorted by cited. The complete set is in all_records.csv / .ris.

### Block PSC (475 unique)
...
- 2 block/backend pair(s) hit the `--limit` cap (300); raise it (largest total 4,579) if you need the complete record set rather than the most-cited slice.
...
- Block MLP: total hits changed from 239 (run 20260907T102936) to 378; count drift is expected as indexes grow, but a large jump usually means the query changed -- diff queries.json between the runs.
